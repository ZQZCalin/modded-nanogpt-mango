...
@torch.compile  #NOTE: not sure if torch.compile will break something unintentionally; turn off if needed
def mango_update(
        acc_bf16_view_u16: Tensor, mantissa: Tensor, momentum_buffer: Tensor, 
        precond_buffer: Tensor | None, postcond_buffer: Tensor | None, grad: Tensor, 
        momentum: Tensor, precond: tuple[Tensor, Tensor, Tensor] | None, postcond: tuple[Tensor, Tensor, Tensor] | None,
        rms_normalize: bool, rms_eps: Tensor,
    ...
    if precond is not None:
        b, p, eps = precond
        precond_buffer.copy_(b * precond_buffer + (1-b) * grad**2)
        v = zeropower_via_newtonschulz5(
            (momentum * momentum_buffer + (1 - momentum) * grad) 
            / ((b * precond_buffer + (1-b) * grad**2)**p + eps)
        )
        #NOTE: for both pre- and post-conditioning, I applied nesterov momentum.
    else:
        v = zeropower_via_newtonschulz5(momentum * momentum_buffer + (1 - momentum) * grad)
    if postcond is not None:
        b, p, eps = postcond
        postcond_buffer.copy_(b * postcond_buffer + (1-b) * grad**2)
        v.div_((b * postcond_buffer + (1-b) * grad**2)**p + eps)
    if rms_normalize:
        v.div_((v**2).mean().sqrt().add_(rms_eps))
...
class Mango(torch.optim.Optimizer):
    """
    Mango optimizer for medium track nanogpt speedrun.

    Update flow:
        SGDM -> pre-conditioning -> Newton-Schulz -> post-conditioning -> rms scaling

    Args:
        precond: tuple of (momentum, power, eps) for pre-conditioning before NS.
        postcond: tuple of (momentum, power, eps) for post-conditioning after NS.
        rms_normalize: bool. normalize final update by rms norm if true.
        rms_eps: epsilon value for rms normalization.
    """
    def __init__(self, params, lr=0.02, momentum=0.95, weight_decay=0.01, 
                 precond=(0.999, 0.5, 1e-8), postcond=None, rms_normalize=True, rms_eps=1e-10, rank=0, world_size=1
    ):
        self.rank = rank
        self.world_size = world_size
        defaults = dict(lr=lr, momentum=momentum, weight_decay=weight_decay,
                        precond=precond, postcond=postcond, rms_normalize=rms_normalize, rms_eps=rms_eps)
        super().__init__(params, defaults)
        assert all(p.dtype == torch.bfloat16 for group in self.param_groups for p in group["params"])
            ...
            for base_i in range(len(params))[::self.world_size]:
                if base_i + self.rank < len(params):
                    ...
                    if len(state) == 0:
                        state["mantissa"] = torch.zeros_like(p, dtype=torch.uint16)
                        state["momentum_buffer"] = torch.zeros_like(p, dtype=torch.float32)
                        state["precond_buffer"] = torch.zeros_like(p, dtype=torch.float32) if group["precond"] is not None else None
                        state["postcond_buffer"] = torch.zeros_like(p, dtype=torch.float32) if group["postcond"] is not None else None
                    mango_update(
                        p.view(torch.uint16), state["mantissa"], 
                        state["momentum_buffer"], state["precond_buffer"], state["postcond_buffer"],
                        p.grad, momentum, group["precond"], group["postcond"],
                        eff_lr=torch._as_tensor_fullprec(group["lr"] * max(1, p.size(-2) / p.size(-1)) ** 0.5),
                        eff_weight_decay=torch._as_tensor_fullprec(group["lr"] * group["weight_decay"] * getattr(p, "wd_mul", 1.0)),
                    )
                futures.append(dist.all_gather(params_pad[base_i:base_i + self.world_size], params_pad[base_i + self.rank], async_op=True).get_future())
...
optimizer2 = Mango(
    hidden_matrix_params, lr=0.025, momentum=0.95,
    precond=(0.999, 0.5, 1e-8), postcond=None, rms_normalize=True, rms_eps=1e-10,
