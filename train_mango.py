# Copied from https://gist.github.com/lapp0/ff6f10c3cd6d0aefb28a49681a44b78c.
# 
...
# NOTE: load data from a different path
# DATA_PATH = "data"    # default
DATA_PATH = "/projectnb/aclab/datasets"
...
# replace muon with mango optimizer.
emb_params = [raw_model.transformer.wte.weight]
head_params = [raw_model.lm_head.weight]
params = list(raw_model.transformer.h.parameters())
matrix_params = [p for p in params if p.ndim == 2]
scalar_params = [p for p in params if p.ndim < 2] + [raw_model.skip_weights]
optimizer1 = Mango(emb_params,      lr=0.6,     beta1=0.8,  beta2=0.95, 
                   nesterov=False, normalize_fn=None, scale_rms=False)
optimizer2 = Mango(head_params,     lr=0.008,   beta1=0.8,  beta2=0.95, 
                   nesterov=False, normalize_fn=None, scale_rms=False)
optimizer3 = Mango(matrix_params,   lr=0.05,    beta1=0.95, beta2=0.0,
                   nesterov=True, normalize_fn="newtonschulz5", scale_rms=False,
                   steps=5, scale_dim=True)
optimizer4 = Mango(scalar_params,   lr=0.04,    beta1=0.8,  beta2=0.95, 
                   nesterov=False, normalize_fn=None, scale_rms=False)
optimizers = [optimizer1, optimizer2, optimizer3, optimizer4]
