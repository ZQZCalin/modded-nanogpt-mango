# The goal of this experiment is to examiine the noise distribution of stochastic gradients.
# Specifically, we'd like to verify whether the noise can be thought of a ill-conditioned distribution
# that is mostly orthogonal to the actual signal (true gradient).
...
def parse_args():
    ...
    parser.add_argument("--momentum", type=str2tuple, default="0.85,0.95,300", help="momentum warmup")
...
optimizer2 = Muon(hidden_matrix_params, lr=cmd_args.lr, momentum=cmd_args.momentum[1], rank=rank, world_size=world_size)
...
for step in range(train_steps + 1):
    ...
    # --------------- CHECKPOINT SECTION -----------------
    if last_step or (step % args.checkpoint_every == 0):
        if master_process and args.save_checkpoint:
            log = dict(step=step, code=code, model=model.state_dict(), optimizers=[opt.state_dict() for opt in optimizers])
            os.makedirs(args.checkpoint_path, exist_ok=False)   # dont allow overwriting existing checkpoints
            torch.save(log, f"{args.checkpoint_path}/state_step{step:06d}.pt")
