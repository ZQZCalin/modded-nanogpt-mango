...
def _(g: Tensor, x_f8: Tensor, w_f8: Tensor, *_):
    # NOTE: could this be the problem why the record is not working?
...
train_loader = distributed_data_generator(
    os.path.join(args.data_dir, args.train_files), world_size * args.seq_len, rank, world_size)
...
for step in range(train_steps + 1):
    ...
    for _ in range(meta_batch_size):
        inputs, targets = next(train_loader)
        loss = model(inputs, targets, window_size_blocks(window_size)) / meta_batch_size
        loss.backward()
